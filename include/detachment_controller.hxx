...
struct DetachmentController : public NamedComponent<DetachmentController> {

  DetachmentController(std::string name, Options& options, Solver*)
      : NamedComponent(name, {readOnly("species:{neutral}:density", Regions::Interior),
    ...
    detachment_front_setpoint = detachment_controller_options["detachment_front_setpoint"]
                                    .doc("How far from the divertor target should the "
                                         "detachment front be? (in m).")
                                    .as<BoutReal>();

    velocity_form =
        detachment_controller_options["velocity_form"]
    ...
    species_for_source_shape = detachment_controller_options["species_for_source_shape"]
                                   .doc("Which species to select the source_shape from?")
                                   .withDefault<std::string>("e");

    neutral_species = detachment_controller_options["neutral_species"]
    ...
    actuator =
        detachment_controller_options["actuator"]
            .doc(
                "What should we adjust to control the detachment front position (options "
                "are 'power' or 'particles' (either main species or impurity))?")
            .as<std::string>();
    if (actuator == "power") {
    ...
    settling_time = detachment_controller_options["settling_time"]
                        .doc("Time taken to allow system to settle before switching on "
                             "certain control terms (in seconds).")
                        .withDefault<BoutReal>(0.0);
    ...
    reset_integral_on_first_crossing =
        detachment_controller_options["reset_integral_on_first_crossing"]
            .doc("Reset the error integral to zero when the detachment front first "
    ...
    species_list = strsplit(
        detachment_controller_options["species_list"]
            .doc("Comma-separated list of species to apply the PI-controlled source to")
            .as<std::string>(),
        ',');
    scaling_factors_list =
        strsplit(detachment_controller_options["scaling_factors_list"]
    ...
    if (species_list.size() != scaling_factors_list.size()) {
      throw BoutException("DetachmentController: species_list length doesn't match "
                          "scaling_factors length. Need 1 scaling factor per species.");
    }

    if (control_mode == control_power) {
      source_shape = (options[std::string("P") + species_for_source_shape]["source_shape"]
                          .doc("Source term in ddt(P" + species_for_source_shape
      ...
      source_shape = (options[std::string("N") + species_for_source_shape]["source_shape"]
                          .doc("Source term in ddt(N" + species_for_source_shape
                               + std::string("). Units [m^-3/s]"))
                          .withDefault(Field3D(0.0)))
                     / (Nnorm * Omega_ci);
    ...
    debug = detachment_controller_options["debug"]
                .doc("Print debugging information to the screen (0 for none, 1 for "
                     "basic, 2 for extensive).")
                .withDefault<int>(0);
  ...
  void outputVars(Options& state) override {
    if (diagnose) {
      ...
      set_with_attrs(state[std::string("detachment_front_location")],
      ...
      set_with_attrs(state[std::string("detachment_control_src_shape")], source_shape,
                     {{"units", source_units},
      ...
      set_with_attrs(state[std::string("detachment_source_feedback")],
                     detachment_source_feedback,
                     {{"time_dimension", "t"},
                      {"units", source_units},
                      {"conversion", source_conversion},
      ...
      set_with_attrs(state[std::string("detachment_control_proportional_term")],
                     proportional_term,
                     {{"time_dimension", "t"},
      ...
      set_with_attrs(state[std::string("detachment_control_integral_term")],
                     integral_term,
                     {{"time_dimension", "t"},
      ...
      set_with_attrs(state[std::string("detachment_control_derivative_term")],
                     derivative_term,
                     {{"time_dimension", "t"},
  ...
  void restartVars(Options& state) override {
    ...
    if ((initialise) && (not ignore_restart)) {
      ...
      if (state.isSet("detachment_control_previous_derivative")) {
        previous_derivative =
            state["detachment_control_previous_derivative"].as<BoutReal>();
      }
      if (state.isSet("detachment_control_number_of_crossings")) {
        number_of_crossings =
            state["detachment_control_number_of_crossings"].as<BoutReal>();
      }
    ...
    set_with_attrs(state["detachment_control_previous_derivative"], previous_derivative,
                   {{"source", "detachment_controller"}});
    set_with_attrs(state["detachment_control_number_of_crossings"], number_of_crossings,
                   {{"source", "detachment_controller"}});
  }

  static constexpr auto type = "detachment_controller";
...
namespace {
RegisterComponent<DetachmentController> register_detachment_controller;
}
