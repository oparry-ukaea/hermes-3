...
struct SimplePump : public NamedComponent<SimplePump> {
  ...
  SimplePump(std::string name, Options& alloptions, Solver*)
    ...
    residence_time =
        (options["residence_time"].doc("Pumping time-constant. Units [s]").as<BoutReal>())
        * Omega_ci;

    sink_shape =
        (options["sink_shape"].doc("Shape of pumping sink.").withDefault(Field3D(0.0)));
...
namespace {
RegisterComponent<SimplePump> register_simple_pump;
}
