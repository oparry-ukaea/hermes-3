...
if "sheath_boundary_simple" in bd.options:
    ...
    (
        ...
        sheath_flow_down,
        sheath_flow_up,
    ) = sheath_boundary_simple(bd, gamma_e, gamma_i, Ne, Te, Ti)
...
for species in ["e", "d+", "d"]:
    # Internal energy
    ddt_Ep += 1.5 * (bd["ddt(P" + species + ")"] * cell_volume).isel(
        x=slice(2, -2), theta=theta_slice
    ).sum(["x", "theta", "zeta"])
    # Kinetic energy
    ddt_Ek += (
        bd["V" + species] * bd["ddt(NV" + species + ")"]
        - 0.5
        * bd["NV" + species]
        * bd["V" + species]
        * bd["ddt(N" + species + ")"]
...
print(
    "Rate of change of energy: {} [{} internal {} kinetic]".format(
        ddt_Ep + ddt_Ek, ddt_Ep, ddt_Ek
    )
)
