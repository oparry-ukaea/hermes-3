...
for grid, paths, paths2 in history:
    ...
    for path in paths:
        ...
        ddt = (
            collect("ddt(" + varname + ")", path=path)[:, 2:-2, :, 0] / scale_timederivs
        )
        ...
        if grid_first:
            ...
            if last_grid_rms is not None:
                ...
                plt.arrow(
                    grid_start / (60 * 60),
                    np.sqrt(last_grid_rms * rms[0]),
                    0,
                    0.1 * np.sqrt(rms[0] * last_grid_rms),
                    length_includes_head=True,
                    color="r",
                    head_width=0.1,
                    head_length=0.2 * np.sqrt(rms[0] * last_grid_rms),
                )
    last_grid_rms = rms[-1]
