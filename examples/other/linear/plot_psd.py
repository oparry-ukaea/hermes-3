...
if var[0] == "V":
    ...
    datasets = [
        collect("N" + var, path=path, xind=xind, yind=yind).squeeze()
        / np.clip(collect(nvar, path=path, xind=xind, yind=yind).squeeze(), 1e-5, None)
        for path in paths
    ]
else:
    datasets = [
        collect(var, path=path, xind=xind, yind=yind).squeeze() for path in paths
    ]
...
f, psd = welch(data[:, 0], fs=1.0 / dt, nperseg=512)
print(psd.shape)

for z in range(1, nz):
    _, p = welch(data[:, z], fs=1.0 / dt, nperseg=512)
    psd += p
psd /= nz
