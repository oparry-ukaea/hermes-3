...
for calls, source, label, linestyle in zip(
    cumulative_calls, source_mags, labels, linestyles
):
...
for path, calls, label, linestyle in zip(paths, cumulative_calls, labels, linestyles):
    for varname, color, varlabel in [
        (r"ddt(Nd+)", "k", r"$\frac{\partial}{\partial t}n_{d+}$"),
