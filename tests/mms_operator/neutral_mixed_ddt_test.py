...
from perpendicular_laplacian import (
    Div_a_Grad_perp_f,
    Div_par_k_Grad_par_f,
    GeneralMetric,
)
...
def neutral_mixed_equations(
    ...
    ddt_Nn = -Div_par(Nn * Vn, metric=metric) - Div_a_Grad_perp_f(
        -Dn * Nn, logPn, metric=metric
    )

    ddt_Pn = (
        -Div_par(Pn * Vn, metric=metric)
        - (5.0 / 3.0) * Div_a_Grad_perp_f(-Dn * Pn, logPn, metric=metric)
        - (2.0 / 3.0) * Pn * Div_par(Vn, metric=metric)
    )

    ddt_NVn = (
        -AA * Div_par(Nn * Vn * Vn, metric=metric)
    ...
    if neutral_viscosity:
        viscosity_source = Div_a_Grad_perp_f(
            Etan, Vn, metric=metric
        ) + Div_par_k_Grad_par_f(Etan, Vn, metric=metric)
        ddt_NVn = ddt_NVn + viscosity_source
        ddt_Pn = ddt_Pn - (2.0 / 3.0) * Vn * viscosity_source
    ...
    test_input = {
        ...
        "sub_test_dir": "evolve_momentum_"
        + str(evolve_momentum)
        + "_conservation_test_"
        + str(conservation_test),
...
test_input = neutral_mixed_equations(
    evolve_momentum=False, conservation_test=False, expected_convergence_order=2.0
)
...
test_input = neutral_mixed_equations(
    evolve_momentum=False, conservation_test=True, expected_convergence_order=2.0
)
success, output_message_1 = run_neutral_mixed_manufactured_solutions_test(test_input)
global_success = success and global_success

test_input = neutral_mixed_equations(
    evolve_momentum=True, conservation_test=False, expected_convergence_order=1.0
)
success, output_message_2 = run_neutral_mixed_manufactured_solutions_test(test_input)
global_success = success and global_success

test_input = neutral_mixed_equations(
    evolve_momentum=True, conservation_test=True, expected_convergence_order=0.8
)
