...
class BraginskiiElectronViscosityTest
...
TEST_F(BraginskiiElectronViscosityTest, ViscosityPressureScaling) {
  ...
  state2["species"]["e"]["pressure"] = 2*state1["species"]["e"]["pressure"].as<Field3D>();

  component.transform(state1);
  component.transform(state2);

  Field3D visc1 = state1["species"]["e"]["momentum_source"], visc2 = state2["species"]["e"]["momentum_source"];
...
TEST_F(BraginskiiElectronViscosityTest, ViscosityCollisionScaling) {
  ...
  state1["species"]["e"]["collision_frequency"] = linearGradient(1., 0.1, 0.2, -0.1, -0.4, 0., 1.);
  state2["species"]["e"]["collision_frequency"] = 2*state1["species"]["e"]["collision_frequency"].as<Field3D>();
...
TEST_F(BraginskiiElectronViscosityTest, ViscosityVelocityScaling) {
  ...
  state1["species"]["e"]["velocity"] = linearGradient(1., 0., 1., 1., 1., 0., 0.);
  state2["species"]["e"]["velocity"] = 2*state1["species"]["e"]["velocity"].as<Field3D>();

  component.transform(state0);
  component.transform(state1);
  component.transform(state2);

  Field3D visc0 = state0["species"]["e"]["momentum_source"], visc1 = state1["species"]["e"]["momentum_source"], visc2 = state2["species"]["e"]["momentum_source"];
  BOUT_FOR_SERIAL(i, visc1.getRegion("RGN_NOBNDRY")) {
