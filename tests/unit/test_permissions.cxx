...
TEST(PermissionsTests, TestCanAccess) {
  ...
  EXPECT_TRUE(example.canAccess("species:he:density"));
  EXPECT_FALSE(example.canAccess("species:he:pressure"));
  EXPECT_TRUE(example.canAccess("species:he:collision_frequency"));
  EXPECT_FALSE(example.canAccess("species:he:velocity"));
  EXPECT_FALSE(example.canAccess("unset"));

  // Check whether we have write permission for the variables across the entire domain
  EXPECT_FALSE(example.canAccess("species:he:density", Permissions::Write));
  EXPECT_FALSE(example.canAccess("species:he:pressure", Permissions::Write));
  EXPECT_TRUE(example.canAccess("species:he:collision_frequency", Permissions::Write));
  EXPECT_FALSE(example.canAccess("species:he:velocity", Permissions::Write));
  EXPECT_FALSE(example.canAccess("unset", Permissions::Write));
  
  // Check whether we have read permission at the boundaries
  EXPECT_TRUE(example.canAccess("species:he:density", Permissions::Read, Permissions::Boundaries));
  EXPECT_FALSE(example.canAccess("species:he:pressure", Permissions::Read, Permissions::Boundaries));
  EXPECT_TRUE(example.canAccess("species:he:collision_frequency", Permissions::Read, Permissions::Boundaries));
  EXPECT_TRUE(example.canAccess("species:he:velocity", Permissions::Read, Permissions::Boundaries));
  EXPECT_FALSE(example.canAccess("unset", Permissions::Read, Permissions::Boundaries));

  // Check permissions set for whole sections
  EXPECT_FALSE(example.canAccess("species:d:pressure"));
  EXPECT_FALSE(example.canAccess("species:d:pressure", Permissions::Write));
  EXPECT_TRUE(example.canAccess("species:d:pressure", Permissions::Write, Permissions::Interior));
  EXPECT_TRUE(example.canAccess("species:d:velocity"));
  EXPECT_FALSE(example.canAccess("species:d:velocity", Permissions::Write));
  EXPECT_TRUE(example.canAccess("species:d:velocity", Permissions::Read, Permissions::Boundaries));
  EXPECT_FALSE(example.canAccess("species:d:collision_frequencies:d_d_coll"));
  EXPECT_FALSE(example.canAccess("species:d:collision_frequencies:d_d_coll", Permissions::Write));
  EXPECT_TRUE(example.canAccess("species:d:collision_frequencies:d_d_coll", Permissions::Write, Permissions::Boundaries));
}
...
std::set<std::pair<std::string, Permissions::Regions>>
make_set(const std::vector<std::pair<std::string, Permissions::Regions>> & vec) {
  return std::set<std::pair<std::string, Permissions::Regions>>(vec.begin(), vec.end());
}
...
TEST(PermissionsTests, TestGetVariablesWithPermissions) {
  ...
  auto read_only = make_set(example.getVariablesWithPermission(Permissions::Read));
  EXPECT_EQ(read_only.size(), 3);
  EXPECT_EQ(read_only.count(make_permissions("species:he:density", Permissions::Interior)), 1);
  EXPECT_EQ(read_only.count(make_permissions("species:he:pressure", Permissions::Boundaries)), 1);
  EXPECT_EQ(read_only.count(make_permissions("species:he:velocity", Permissions::Boundaries)), 1);

  auto readable = make_set(example.getVariablesWithPermission(Permissions::Read, false));
  EXPECT_EQ(readable.size(), 4);
  EXPECT_EQ(readable.count(make_permissions("species:he:density", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:he:pressure", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:he:collision_frequency", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:he:velocity", Permissions::Boundaries)), 1);

  auto write_nonfinal = make_set(example.getVariablesWithPermission(Permissions::Write, true));
  EXPECT_EQ(write_nonfinal.size(), 1);
  EXPECT_EQ(write_nonfinal.count(make_permissions("species:he:pressure", Permissions::Interior)), 1);

  auto writable = make_set(example.getVariablesWithPermission(Permissions::Write, false));
  EXPECT_EQ(writable.size(), 3);
  EXPECT_EQ(writable.count(make_permissions("species:he:density", Permissions::Boundaries)), 1);
  EXPECT_EQ(writable.count(make_permissions("species:he:pressure", Permissions::Interior)), 1);
  EXPECT_EQ(writable.count(make_permissions("species:he:collision_frequency", Permissions::AllRegions)), 1);

  auto final_write = make_set(example.getVariablesWithPermission(Permissions::Final));
  EXPECT_EQ(final_write.size(), 2);
  EXPECT_EQ(final_write.count(make_permissions("species:he:density", Permissions::Boundaries)), 1);
  EXPECT_EQ(final_write.count(make_permissions("species:he:collision_frequency", Permissions::AllRegions)), 1);
}
...
TEST(PermissionsTests, TestSubstitute) {
  ...
  EXPECT_EQ(readable.count(make_permissions("{var}", Permissions::Interior)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:e:collision_frequencies:e_e_coll", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:e:collision_frequencies:e_d+_coll", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:d+:collision_frequencies:d+_e_coll", Permissions::AllRegions)), 1);
  EXPECT_EQ(readable.count(make_permissions("species:d+:collision_frequencies:d+_d+_coll", Permissions::AllRegions)), 1);

  example.substitute("var", {"a", "b", "c"});
  
  auto writable = make_set(example.getVariablesWithPermission(Permissions::Write));
  EXPECT_EQ(writable.size(), 3);
  EXPECT_EQ(writable.count(make_permissions("a", Permissions::Interior)), 1);
  EXPECT_EQ(writable.count(make_permissions("b", Permissions::Interior)), 1);
  EXPECT_EQ(writable.count(make_permissions("c", Permissions::Interior)), 1);
}
