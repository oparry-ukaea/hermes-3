...
TEST_F(IonisationTest, SourcesSanityCheck) {
  ...
  BOUT_FOR_SERIAL(i, h_density_source.getRegion("RGN_NOBNDRY")) {
    ASSERT_LT(h_density_source[i], 0.0)
        << "Atom (h) density source not negative at " << i;
    ASSERT_GT(hp_density_source[i], 0.0)
        << "Ion (h+) density source not positive at " << i;
  }
  ...
  BOUT_FOR_SERIAL(i, h_momentum_source.getRegion("RGN_NOBNDRY")) {
    ASSERT_LT(h_momentum_source[i], 0.0)
        << "Atom (h) momentum source not negative at " << i;
    ASSERT_GT(hp_momentum_source[i], 0.0)
        << "Ion (h+) momentum source not positive at " << i;
  }
  ...
  BOUT_FOR_SERIAL(i, h_energy_source.getRegion("RGN_NOBNDRY")) {
    ASSERT_LT(e_energy_source[i], 0.0)
        << "Electron (e) energy source not negative at " << i;
  }
}
