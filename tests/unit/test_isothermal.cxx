...
TEST_F(IsothermalTest, GivenTandN) {
  ...
  BOUT_FOR_SERIAL(i, Te.getRegion("RGN_ALL")) { ASSERT_DOUBLE_EQ(Te[i], 3.0); }

  auto Pe = get<Field3D>(state["species"]["e"]["pressure"]);

  BOUT_FOR_SERIAL(i, Pe.getRegion("RGN_ALL")) { ASSERT_DOUBLE_EQ(Pe[i], 6.0); }
}
...
TEST_F(IsothermalTest, Outputs) {
  ...
  Options outputs = {{"Tnorm", 1.0}, {"Nnorm", 1.0}};
