...
class BraginskiiIonViscosityTest
...
TEST_F(BraginskiiIonViscosityTest, ViscosityPressureScaling) {
  ...
  state2["species"]["d+"]["pressure"] = 2*state1["species"]["d+"]["pressure"].as<Field3D>();

  component.transform(state1);
  component.transform(state2);

  Field3D visc1 = state1["species"]["d+"]["momentum_source"], visc2 = state2["species"]["d+"]["momentum_source"];
...
TEST_F(BraginskiiIonViscosityTest, ViscosityCollisionScaling) {
  ...
  state2 = state1.copy();
  state1["species"]["d+"]["collision_frequencies"]["d+_d+_coll"] = linearGradient(1., 0.1, 0.2, 0.1, 0.4, 1., 0.2);
  state1["species"]["d+"]["collision_frequencies"]["d+_he+_coll"] = 0.5;
  state2["species"]["d+"]["collision_frequencies"]["d+_d+_coll"] = 2*state1["species"]["d+"]["collision_frequencies"]["d+_d+_coll"].as<Field3D>();
  state2["species"]["d+"]["collision_frequencies"]["d+_he+_coll"] = 2*state1["species"]["d+"]["collision_frequencies"]["d+_he+_coll"].as<Field3D>();
...
TEST_F(BraginskiiIonViscosityTest, ViscosityVelocityScaling) {
  ...
  state1["species"]["d+"]["velocity"] = linearGradient(1., 0., 1., 1., 1., 0., 0.);
  state2["species"]["d+"]["velocity"] = 2*state1["species"]["d+"]["velocity"].as<Field3D>();

  component.transform(state0);
  component.transform(state1);
  component.transform(state2);

  Field3D visc0 = state0["species"]["d+"]["momentum_source"], visc1 = state1["species"]["d+"]["momentum_source"], visc2 = state2["species"]["d+"]["momentum_source"];
  BOUT_FOR_SERIAL(i, visc1.getRegion("RGN_NOBNDRY")) {
...
TEST_F(BraginskiiIonViscosityTest, ViscosityCollisionMode) {
  ...
  Field3D visc1 = state1["species"]["d+"]["momentum_source"], visc2 = state2["species"]["d+"]["momentum_source"];
  BOUT_FOR_SERIAL(i, visc1.getRegion("RGN_NOBNDRY")) {
    // Collision frequency calculated using multispecies mode is twice that calculated with Braginskii mode, so viscosity should be half.
    ASSERT_NE(visc1[i], 0.);
    ASSERT_FLOAT_EQ(2*visc1[i], visc2[i]);
  }
}
