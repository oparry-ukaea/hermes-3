...
TEST_F(FixedFractionIonsTest, MissingFractions) {
  ...
  ASSERT_THROW(FixedFractionIons component("test", options, nullptr), BoutException);
}
...
TEST_F(FixedFractionIonsTest, SingleIon) {
  ...
  ASSERT_TRUE(IsFieldEqual(state["species"]["h"]["density"].as<Field3D>(), 0.6 * 1.4,
                           "RGN_NOBNDRY"));
}
...
TEST_F(FixedFractionIonsTest, TwoIons) {
  ...
  ASSERT_TRUE(IsFieldEqual(state["species"]["ne"]["density"].as<Field3D>(), 0.05 * 1.4,
                           "RGN_NOBNDRY"));
}
